import { ReqParams } from "./getters/getAllCloses";
...
export function decodeProps(props: ReqParams) {
  ...
  if (Object.values(props).some(value => typeof value !== 'string' && typeof value !== 'number')) {
  decodedProps = JSON.parse(decodeURIComponent(JSON.stringify(props)));
  }

  decodedProps = {
    ...decodedProps,
    location: typeof decodedProps.location === "string"
